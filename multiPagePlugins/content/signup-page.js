...
const {
  isResendVerificationButtonLabel = () => false,
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (
    ...
    || message.type === 'RESEND_CODE'
    || message.type === 'STEP8_FIND_AND_CLICK'
    || message.type === 'RETRY_ERROR_PAGE'
  ) {
    resetStopState();
    handleCommand(message).then((result) => {
      sendResponse({ ok: true, ...(result || {}) });
...
function getCurrentPageState() {
  ...
  const retryButton = findVisibleErrorRetryButton();
  const errorPage = detectOpenAiErrorPage();
  const phoneRequiredPage = detectPhoneRequiredPage();

  return {
    url: location.href,
    hasVisibleContinueButton,
    hasVisibleRetryButton: Boolean(retryButton),
...
async function step3_fillEmailPassword(payload) {
  ...
  await fillInputAndVerify(emailInput, email, 'signup email');
  ...
  await fillInputAndVerify(passwordInput, payload.password, 'signup password');
...
function detectPhoneRequiredPage() {
  const bodyText = normalizeVisibleText(document.body?.innerText || '');
  const phoneRequiredText = /电话号码是必填项|请继续添加电话号码|电话号?码是必填项|phone number is required|required phone number|add your phone number/i;
  if (!phoneRequiredText.test(bodyText)) {
    return false;
  }

  const phoneInput = Array.from(document.querySelectorAll('input')).find((input) => {
    ...
    return /tel|phone|电话号码|电话号|手机号/.test(text);
  });

  const continueButton = Array.from(document.querySelectorAll('button, [role="button"], input[type="submit"]')).find((button) => {
...
async function resendVerificationCode(step) {
  log(`Step ${step}: Looking for resend verification button...`);

  const resendButton = await waitForVerificationResendButtonEnabled();
  const buttonText = getActionLabel(resendButton);

  await humanPause(350, 900);
  simulateClick(resendButton);
  log(`Step ${step}: Clicked resend verification button (${buttonText || 'unlabeled control'})`);
...
async function retryErrorPage() {
  const errorPage = detectOpenAiErrorPage();
  if (!errorPage) {
    throw new Error('Current page is not a recognized OpenAI error page.');
  }

  const retryButton = findVisibleErrorRetryButton();
  if (!retryButton) {
    throw new Error('Could not find Retry button on error page. URL: ' + location.href);
  }

  await humanPause(350, 900);
  simulateClick(retryButton);
  log(`Error page: Clicked retry button (${getActionLabel(retryButton) || 'retry'})`);
...
async function step6_login(payload) {
  ...
  await humanPause(500, 1400);
  await fillInputAndVerify(emailInput, email, 'login email');
  ...
  if (passwordInput) {
    ...
    await fillInputAndVerify(passwordInput, password, 'login password');
...
function detectOpenAiErrorPage() {
  const titleText = normalizeVisibleText(document.body?.innerText || '');
  const hasErrorTitle = /糟糕，?出错了|something went wrong|oops[,，]?\s*there was an error/i.test(titleText);
  const hasTimeout = /operation timed out|请求超时|操作超时/i.test(titleText);
  const retryButton = findVisibleErrorRetryButton();
  if (!retryButton && !hasErrorTitle && !hasTimeout) {
    return null;
  }
  if (hasErrorTitle || hasTimeout) {
    return { message: extractErrorMessage(titleText) };
  }
  return null;
}

function findVisibleErrorRetryButton() {
  const candidates = document.querySelectorAll('button, a, [role="button"], [role="link"]');
  for (const candidate of candidates) {
    if (!isElementVisible(candidate)) continue;
    if (/^重试$|^retry$/i.test(normalizeVisibleText(getActionLabel(candidate)))) {
      return candidate;
    }
  }
  return null;
...
function extractErrorMessage(text) {
  ...
  const maxCheckAttemptsMatch = normalized.match(/max_check_attempts/i);
  if (maxCheckAttemptsMatch) {
    return maxCheckAttemptsMatch[0];
  }
  const timeoutMatch = normalized.match(/Operation timed out|请求超时|操作超时/i);
  if (timeoutMatch) {
    return timeoutMatch[0];
  }
  const titleMatch = normalized.match(/糟糕，?出错了|something went wrong|oops[,，]?\s*there was an error/i);
  return titleMatch ? titleMatch[0] : normalized.slice(0, 120);
}

function findVisibleVerificationResendButton() {
  const selectorMatches = document.querySelectorAll(
    'button[name="intent"][value="resend"], button[type="submit"][name="intent"][value="resend"]'
  );
  ...
  for (const candidate of candidates) {
    ...
    if (isResendVerificationButtonLabel(getActionLabel(candidate))) {
      return candidate;
    }
  }
...
async function waitForVerificationResendButtonEnabled(timeout = 12000) {
  const start = Date.now();
  ...
  while (Date.now() - start < timeout) {
    ...
    const button = findVisibleVerificationResendButton();
    if (button) {
  ...
  if (sawDisabledButton) {
    throw new Error('Resend verification button stayed disabled for too long. URL: ' + location.href);
  }

  throw new Error('Could not find a resend verification button. URL: ' + location.href);
}
...
async function fillInputAndVerify(el, value, label = 'input', attempts = 3) {
  for (let attempt = 1; attempt <= attempts; attempt++) {
    ...
    log(`${label}: value mismatch after fill attempt ${attempt}/${attempts}. Expected "${value}", got "${currentValue}". Retrying...`, 'warn');
    await sleep(150);
  }

  const finalValue = typeof el.value === 'string' ? el.value : '';
  if (finalValue !== value) {
    throw new Error(`${label} value was overwritten unexpectedly. Expected "${value}", got "${finalValue}". URL: ${location.href}`);
  }
}
